    ...
    <main className="bg-[#0a0e27] text-white min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 py-12 lg:py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10">
          {apps.map((app: any) => {
            const slug = typeof app.slug === 'object' ? app.slug?.current : app.slug
            const status = statusConfig[app.status] || statusConfig.development
            return (
              <Link
                key={app.id}
                href={`/apps/${slug}`}
                ...
                <div className="group cursor-pointer">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {app.features?.slice(0, 4).map((feature: string) => (
                        <span
                          key={feature}
